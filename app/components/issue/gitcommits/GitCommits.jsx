          ...
          { collection.length <= 0 && !indexLoading ?
            ...
            _.map(collection, (val, i) => {
              const header = ( <div style={ { fontSize: '12px' } }>
                <span dangerouslySetInnerHTML= { { __html: '<a title="' + (val.author && (val.author.name + '(' + val.author.email + ')')) + '">' + (val.author && val.author.id === currentUser.id ? ' me' : val.author.name) + '</a> 提交代码 - ' + (this.state.displayTimeFormat == 'absolute' ? moment.unix(val.committed_at).format('YYYY/MM/DD HH:mm:ss') : getAgoAt(val.committed_at, currentTime)) } } />
              </div> ); 
