          ...
          { collection.length <= 0 && !indexLoading ?
            ...
            _.map(collection, (val, i) => {
              const header = ( <div style={ { fontSize: '12px' } }>
                <span dangerouslySetInnerHTML= { { __html: '<a title="' + (val.operator && (val.operator.name + '(' + val.operator.email + ')')) + '">' + (val.operator && val.operator.id === currentUser.id ? ' me' : val.operator.name) + '</a> ' + (val.operation == 'modify' ? '修改': 'New') + ' - ' + (this.state.displayTimeFormat == 'absolute' ? moment.unix(val.operated_at).format('YYYY/MM/DD HH:mm:ss') : getAgoAt(val.operated_at, currentTime)) } } />
              </div> ); 
