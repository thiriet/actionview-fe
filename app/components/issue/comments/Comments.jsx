                     ...
                     { _.map(val.reply, (v, i) => {
                       ...
                       <li className='reply-contents-item'>
                         <div className='reply-item-header'>
                           <span dangerouslySetInnerHTML= { { __html: '<a title="' + (v.creator && (v.creator.name + '(' + v.creator.email + ')')) + '">' + (v.creator && v.creator.id === currentUser.id ? ' me' : v.creator.name) + '</a> - ' + (this.state.displayTimeFormat == 'absolute' ? moment.unix(v.created_at).format('YYYY/MM/DD HH:mm:ss') : getAgoAt(v.created_at, currentTime)) + (v.edited_flag == 1 ? '<span style="color:red"> - Edited</span>' : '') } }/>
